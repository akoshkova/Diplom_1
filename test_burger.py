...
def test_add_ingredient(burger, sample_ingredients, ingredient_count, expected_length):
    for i in range(ingredient_count):
        burger.add_ingredient(sample_ingredients[i % len(sample_ingredients)])
    assert len(burger.ingredients) == expected_length


def test_remove_ingredient(burger, sample_ingredients):
...
def test_move_ingredient(burger, sample_ingredients):
    for ingredient in sample_ingredients:
        burger.add_ingredient(ingredient)
...
def test_get_price(burger, test_bun, sample_ingredients, mocker):
...
def test_get_receipt(burger, test_bun, sample_ingredients, mocker):
...
def test_remove_from_empty_burger(burger):
    with pytest.raises(IndexError):
        burger.remove_ingredient(0)


def test_move_invalid_indexes(burger, sample_ingredients):
    burger.add_ingredient(sample_ingredients[0])
    with pytest.raises(IndexError):
